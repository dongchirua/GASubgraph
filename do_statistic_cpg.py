...
def explainers(blackbox_model, classifier, sample, k_node, device, n_generation, CXPB, MUTPB, tournsize,
    ...
    _, gnn_edge_mask = gnn_explainer.explain_graph(
        sample.x.to(device), sample.edge_index.to(device), batch=None)
    end = time.time()
...
def get_method(checkpoint_path):
...
def load_data(data_dir, queue, event, psend_pipe,
              to_undirected, seed, gtype, wait_time=0.1, ):
    reveal_dataset = Reveal(data_dir, gtype, to_undirected=to_undirected, seed=seed)
...
if __name__ == "__main__":
    ...
    output_path = Path(f'cpg_statistic_{node_constraint}.tsv')
    n_handler_workers = 4
    ...
    reader_process = mp.Process(
        target=load_data,
        args=(data_dir, queue, event, psend_pipe, args.to_undirected, args.seed, args.gtype)
    )
    detector_processes = [
        mp.Process(target=main, args=(queue, event, get_method(checkpoint), device, lock, output_path, node_constraint,
